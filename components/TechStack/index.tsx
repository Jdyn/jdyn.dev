    ...
    ({ args: [index], active, movement: [mx], direction: [xDir], velocity: [vx] }) => {
      ...
      api.start((i) => {
        ...
        const rotation = active
          ? 0
          : Math.floor(Math.random() * 30) + mx / 100 + (isGone ? dir * 10 * vx : 0);
  ...
  return (
    <section className={styles.root}>
      ...
      {springs.map((props, index): JSX.Element => {
        ...
        return (
          <animated.div className={styles.container} key={cards[index].name} style={props}>
            <animated.div
            ...
            >
              <div className={styles.cardHeader}>
                <Image alt={card.name} src={card.icon} width={125} height={125} />
                <h2>{card.name}</h2>
              </div>
